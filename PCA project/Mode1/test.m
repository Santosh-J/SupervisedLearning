...
for i = 1:class
    fn = strcat('ATT_faces\s',  num2str(i), '\');
    B = [];
    for j = 1:Trainingimages
...
%Training_labels for the training data
Training_labels = [];
for i = 1:class
    for j = 1:Trainingimages
...
% %%Predicted_labels for the test data
% Predicted_labels = [];
% for i = 1:No_of_class
%     for j = 6:10
...
T = [];
for i = 1:class
    fn = strcat('\\kc.umkc.edu\kc-users\home\s\sjzkt\My Documents\MATLAB\ATT_faces\s', num2str(i) , '\');
    S = [];
    for j = 6:10
        f = strcat(num2str(j), '.pgm');
